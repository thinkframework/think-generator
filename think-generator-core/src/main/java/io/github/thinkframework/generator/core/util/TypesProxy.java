 ...
 * @author hdhxby
 */
public class TypesProxy {
    private static final String TYPTES = "java.sql.Types.";
    ...
    public TypesProxy(GeneratorConfiguration generatorConfiguration) {
        generatorConfiguration.getConverts().forEach((key,value) -> {
            if(key.startsWith(TYPTES)) {
                try {
                    map.put(Types.class.getField(key.substring(TYPTES.length())).getInt(Types.class),Class.forName(value));
    ...
    public Class dataType(Integer dataType) {
        return map.computeIfAbsent(dataType,(kev) -> TypesMapping.getInstance().dataType(kev));
    }
